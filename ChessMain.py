...
from arm_controller import setup_serial, send_move_to_arm, close_serial
...
def main():
    ...
    ser = setup_serial()  # Setup the serial connection
    ...
    while running:
        ...
        if not game_over and not human_turn and not move_undone:
            ...
            if not move_finder_process.is_alive():
                ...
                animate = True
                ai_thinking = False
                send_move_to_arm(ser, ai_move)  # Send the AI move to the robotic arm
    ...
    close_serial(ser)  # Ensure the serial port is closed
...
def drawMoveLog(screen, game_state, font):
    ...
    for i in range(0, len(move_log), 2):
        move_string = str(i // 2 + 1) + ". " + move_log[i].getChessNotation() + " "
        if i + 1 < len(move_log):
            move_string += move_log[i + 1].getChessNotation() + "  "
