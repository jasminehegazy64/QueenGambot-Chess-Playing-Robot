...
MAX_TIME_PER_PLAYER = 10  # 10 seconds bagarab
...
def main():
    ...
    # Inside the main function, initialize a single timer for each player
    player_one_timer = MAX_TIME_PER_PLAYER
    player_two_timer = MAX_TIME_PER_PLAYER

    # Initialize variable to keep track of current player's turn
    current_player = "white" if player_one else "black"

    # Flag to track if the game has started
    game_started = False

    while running:
        # Update timer if the game has started
        if game_started and not game_over:
            if game_state.white_to_move:
                player_one_timer -= 1 / MAX_FPS
                if player_one_timer <= 0:
                    player_one_timer = 0
                    game_over = True
            else:
                player_two_timer -= 1 / MAX_FPS
                if player_two_timer <= 0:
                    player_two_timer = 0
                    game_over = True

            if player_one_timer <= 0 or player_two_timer <= 0:
                drawEndGameText(screen, "Time's up!")
                game_over = True
        ...
        for e in p.event.get():
            ...
            if e.type == p.MOUSEBUTTONDOWN:
                # If the game hasn't started yet, start the game timer
                if not game_started:
                    game_started = True
        ...
        drawGameState(screen, game_state, valid_moves,
                      square_selected, player_one_timer, player_two_timer)
...
def drawGameState(screen, game_state, valid_moves, square_selected, player_one_timer, player_two_timer):
    """
    ...
    timer_text_position = (
        BOARD_WIDTH - player_one_timer_text.get_width() - 10, BOARD_HEIGHT - 50)
    screen.blit(player_one_timer_text, timer_text_position)
    # Place the second timer below the first one
    screen.blit(player_two_timer_text,
                (timer_text_position[0], timer_text_position[1] + 30))
